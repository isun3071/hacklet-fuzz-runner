...
This project was extracted from the `hacklet-league` monorepo without its git history. That history —
130 commits over three intense days (2026-07-08 → 07-10) — is distilled here as narrative, with the raw
dated commit subjects as an appendix. It's the *why* behind the code; when something looks deliberate, it
probably is.
...
*130 commits, 2026-07-08 → 2026-07-10, from the `hacklet-league` monorepo (paths under `fuzz-runner/`).*
```
